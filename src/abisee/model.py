...
class SummarizationModel(object):
    ...
    def __init__(self, hps, vocab: Vocab):
        self._hps = hps
        self._vocab = vocab
    ...
    def _make_feed_dict(self, batch: Batch, just_enc=False):
    ...
    def _add_decoder(self, inputs):
        ...
        outputs, out_state, attn_dists, p_gens, coverage = attention_decoder(
            ...
            # In decode mode, we run attention_decoder one step at a time and so need to pass in the previous
            # step's coverage vector each time
        )
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists):
        ...
        with tf.variable_scope("final_distribution"):
            ...
            indices = tf.stack((batch_nums, self._enc_batch_extend_vocab), axis=2)  # (batch_size, enc_t, 2)
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope("seq2seq"):
            ...
            with tf.variable_scope("embedding"):
                ...
                # tensor with shape (batch_size, max_enc_seq_len, emb_size)
                emb_enc_inputs = tf.nn.embedding_lookup(emb, self._enc_batch)
                # list length max_dec_steps containing shape (batch_size, emb_size)
                emb_dec_inputs = [tf.nn.embedding_lookup(emb, x) for x in tf.unstack(self._dec_batch, axis=1)]
            ...
            with tf.variable_scope("output_projection"):
                ...
                # vocab_scores is the vocabulary distribution before applying softmax. Each entry on the list
                # corresponds to one decoder step
                vocab_scores = []
                ...
                # The vocabulary distributions. List length max_dec_steps of (batch_size, vsize) arrays. The
                # words are in the order they appear in the vocabulary file.
                ...
                with tf.variable_scope("loss"):
                    if FLAGS.pointer_gen:
                        ...
                        for dec_step, dist in enumerate(final_dists):
                            targets = self._target_batch[:, dec_step]  # indices of target words (batch_size)
                            indices = tf.stack((batch_nums, targets), axis=1)  # shape (batch_size, 2)
                            gold_probs = tf.gather_nd(dist, indices)  # prob of correct words on this step
...
def _mask_and_avg(values: list[tf.Tensor], padding_mask):
    """Applies mask to values then returns overall average (a scalar) per batch. First each example's losses are averaged over its timesteps and then overall average is calculated from each example loss.
...
def _coverage_loss(attn_dists, padding_mask):
    ...
    covlosses = []
    for a in attn_dists:
        covloss = tf.reduce_sum(tf.minimum(a, coverage), axis=1)  # calculate the coverage loss for this step
