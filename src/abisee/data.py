...
PAD_TOKEN = "[PAD]"  # This has a vocab id, used to pad the encoder input, decoder input and target sequence
UNKNOWN_TOKEN = "[UNK]"  # This has a vocab id, which is used to represent out-of-vocabulary words
START_TOKEN = "[START]"  # This has a vocab id, which is used at the start of every decoder input sequence
STOP_TOKEN = "[STOP]"  # This has a vocab id, which is used at the end of untruncated target sequences
...
class Vocab(object):
    ...
    def __init__(self, vocab_file: str, max_size: int):
        ...
        for w in [UNKNOWN_TOKEN, PAD_TOKEN, START_TOKEN, STOP_TOKEN]:
        ...
        with open(vocab_file, "r") as vocab_f:
            for line in vocab_f:
                ...
                if len(pieces) != 2:
                    ...
                    continue
                w = pieces[0]
                if w in [SENT_START, SENT_END, UNKNOWN_TOKEN, PAD_TOKEN, START_TOKEN, STOP_TOKEN]:
                    raise Exception(
...
def article2ids(article_words: list[str], vocab: Vocab) -> tuple[list[int], list[str]]:
    ...
    for w in article_words:
        ...
        if i == unk_id:  # If w is OOV
            ...
            ids.append(vocab.size() + oov_num)  # This is e.g. 50000 for the first article OOV, 50001 for 2nd
        else:
            ids.append(i)
    return ids, oovs
...
def abstract2sents(abstract: str) -> list[str]:
    ...
    while True:
        try:
            start_p = abstract.index(SENT_START, cur)
